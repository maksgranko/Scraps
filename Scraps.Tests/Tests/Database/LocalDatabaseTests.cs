...
namespace Scraps.Tests.DatabaseLayer
{
    ...
    public class LocalDatabaseTests : IDisposable
    {
        ...
        public void Schema_GetTableSchema_Works()
        {
            ...
            var columnNames = dt.Rows.Cast<System.Data.DataRow>().Select(r => r["ColumnName"].ToString()).ToList();
            Assert.Contains("OrderID", columnNames);
        ...
        public void Data_FindByColumn_ExactMatch_Works()
        {
            ...
            var found = data.FindByColumn("Books", "Title", "C# in Depth", op: SqlFilterOperator.Eq);
            Assert.Single(found.Rows);
            Assert.Equal("C# in Depth", found.Rows[0]["Title"]);

            var notFound = data.FindByColumn("Books", "Title", "Missing", op: SqlFilterOperator.Eq);
        ...
        public void Data_FindByColumn_PartialMatch_Works()
        {
            ...
            var found = data.FindByColumn("Movies", "Title", "Mat", op: SqlFilterOperator.Like);
            Assert.Single(found.Rows);
        }
